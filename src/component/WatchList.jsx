...
export default function WatchList() {
  const { state , handleToRemoveWatchList , handleMoveToWatchedList } = useContext(MovieContext);
  return (
    <div>
      <h1>WatchList</h1>
            ...
            state.watchList.map((movieItem) =>
              <div className="movie-card" key={movieItem.id}>
                ...
                <div className="buttons-wrapper">
                  <button onClick={() => handleToRemoveWatchList(movieItem.id)} >Remove from  WatchList</button>
                  ...
                  <button onClick={() => handleMoveToWatchedList(movieItem)}>Move To Watched List </button>
                </div>
              </div>
      ...
      <h1>There is no any movie in the watch Add one </h1>
        }
    </div>
    </div >
  )
